...
export const SessionHistory = ({ onStartPractice }) => {
  ...
  const loadSessions = () => {
    ...
    try {
      ...
      console.log('SessionHistory loaded sessions:', allSessions.length);
    } catch (error) {
            ...
            <Card key={session.id} sx={{ mb: 2 }}>
              <CardContent>
                ...
                <Box display="flex" gap={1} mb={2} flexWrap="wrap">
                  <Chip
                    label={`Grammar: ${session.feedback.grammarFeedback.score.toFixed(1)}`}
                    size="small"
                    color={getScoreColor(session.feedback.grammarFeedback.score)}
                    variant="outlined"
                  />
                  <Chip
                    label={`Vocabulary: ${session.feedback.vocabularyFeedback.score.toFixed(1)}`}
                    size="small"
                    color={getScoreColor(session.feedback.vocabularyFeedback.score)}
                    variant="outlined"
                  />
                  <Chip
                    label={`Fluency: ${session.feedback.fluencyFeedback.score.toFixed(1)}`}
                    size="small"
                    color={getScoreColor(session.feedback.fluencyFeedback.score)}
                    variant="outlined"
                  />
                </Box>
