...
export const AnimatedProgressRing: React.FC<AnimatedProgressRingProps> = ({
  ...
  useEffect(() => {
    ...
    // Cubic bezier easing function (matching design spec)
...
export const ScoreRing: React.FC<{ score: number; label?: string; size?: number }> = ({ 
  score, 
  label = "Score", 
  size = 120 
}) => (
...
export const MiniScoreRing: React.FC<{ score: number; size?: number }> = ({ 
  score, 
  size = 60 
}) => (
  <AnimatedProgressRing 
...
export const SkillProgressRing: React.FC<{ 
  value: number; 
  maxValue?: number; 
  label: string;
  color?: 'primary' | 'success' | 'warning' | 'error';
